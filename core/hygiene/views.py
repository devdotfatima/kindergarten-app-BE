...
class HygieneViewSet(ModelViewSet):
    ...
    permission_classes = [IsAuthenticated, CanManageHygieneActivities]
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            openapi.Parameter(
                ...
                description="Filter hygiene by child ID",
                type=openapi.TYPE_INTEGER
            ),
            openapi.Parameter(
                "date", openapi.IN_QUERY,
                description="Filter hygiene by date (YYYY-MM-DD)",
                type=openapi.TYPE_STRING,
                format=openapi.FORMAT_DATE
            ),
        ],
        responses={200: HygieneSerializer(many=True)}
    )
    ...
    def get_queryset(self):
        """Filter hygiene based on role and query parameters (child & date)."""
        user = self.request.user
        if not user.is_authenticated:
          return Hygiene.objects.none()
