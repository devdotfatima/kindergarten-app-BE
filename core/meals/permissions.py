...
class CanManageMeals(BasePermission):
    """
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if user.role == "admin":
            # Admin can manage meals only for children in their kindergarten
            return hasattr(user, "kindergarten_admin") and obj.child.kindergarten == user.kindergarten_admin.kindergarten

        if user.role == "teacher":
            # Teachers can manage meals only for children in their assigned class
