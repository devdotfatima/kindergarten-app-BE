...
class MealViewSet(ModelViewSet):
    ...
    permission_classes = [IsAuthenticated, CanManageMeals]
    ...
    @swagger_auto_schema(
        operation_description="Retrieve meals based on role and filters (child, date).",
        manual_parameters=[
            openapi.Parameter(
                "child_id", openapi.IN_QUERY,
                description="Filter meals by child ID",
    ...
    def list(self, request, *args, **kwargs):
        """Filter meals by child and date if requested."""
        return super().list(request, *args, **kwargs)

    def perform_create(self, serializer):
        """Ensure the requesting user has permission to add a meal for this child."""
    ...
    def get_queryset(self):
        """Filter meals based on role and query parameters (child & date)."""
        ...
        if meal_date:
            queryset = queryset.filter(date=meal_date)
